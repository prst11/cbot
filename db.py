...
def init_db():
    with get_conn() as conn:
        ...
        for coldef in [
            ...
            "last_summary_date TEXT NOT NULL DEFAULT ''",
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS fired_alerts_log (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        """)
        try:
            conn.execute("ALTER TABLE portfolio ADD COLUMN bucket TEXT NOT NULL DEFAULT 'holding'")
        except sqlite3.OperationalError:
            pass
        conn.execute("""
        ...
        """)
        try:
            conn.execute("ALTER TABLE payments ADD COLUMN plan_days INTEGER NOT NULL DEFAULT 30")
        except sqlite3.OperationalError:
            pass
        conn.execute("""
...
def get_theme(user_id: int) -> str:
    with get_conn() as conn:
        ...
        return row["theme"] if row and row["theme"] else "dark"
...
# ─── Лог сработавших алертов (для бейджа-счётчика в Mini App) ───
def log_fired_alert(user_id: int, coin: str, text: str):
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO fired_alerts_log (user_id, coin, text, seen, created_at) VALUES (?, ?, ?, 0, ?)",
            (user_id, coin, text, int(time.time())),
        )


def unseen_alert_count(user_id: int) -> int:
    with get_conn() as conn:
        row = conn.execute(
            "SELECT COUNT(*) c FROM fired_alerts_log WHERE user_id = ? AND seen = 0", (user_id,)
        ).fetchone()
        return row["c"]


def mark_alerts_seen(user_id: int):
    with get_conn() as conn:
        conn.execute("UPDATE fired_alerts_log SET seen = 1 WHERE user_id = ? AND seen = 0", (user_id,))


# ─── Портфель ───
def count_portfolio(user_id: int) -> int:
    """Общее число позиций во всех корзинах — на это считается бесплатный лимит."""
    with get_conn() as conn:
        row = conn.execute("SELECT COUNT(*) c FROM portfolio WHERE user_id = ?", (user_id,)).fetchone()
        return row["c"]


def get_portfolio(user_id: int, bucket: str | None = None) -> list[sqlite3.Row]:
    with get_conn() as conn:
        if bucket:
            return conn.execute(
                "SELECT * FROM portfolio WHERE user_id = ? AND bucket = ? ORDER BY id", (user_id, bucket)
            ).fetchall()
        return conn.execute("SELECT * FROM portfolio WHERE user_id = ? ORDER BY id", (user_id,)).fetchall()


def add_portfolio_item(user_id: int, coin: str, amount: float, bucket: str = "holding"):
    with get_conn() as conn:
        existing = conn.execute(
            "SELECT id, amount FROM portfolio WHERE user_id = ? AND coin = ? AND bucket = ?",
            (user_id, coin, bucket),
        ).fetchone()
        if existing:
            conn.execute(
            ...
            conn.execute(
                "INSERT INTO portfolio (user_id, coin, amount, bucket) VALUES (?, ?, ?, ?)",
                (user_id, coin, amount, bucket),
            )
...
def users_for_daily_summary(today: str) -> list[int]:
    """
    Юзеры, у которых есть хотя бы одна монета в портфеле и кому сегодня
    сводку ещё не отправляли (last_summary_date != today).
    """
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT DISTINCT u.user_id FROM users u
            JOIN portfolio p ON p.user_id = u.user_id
            WHERE u.last_summary_date != ?
        """, (today,)).fetchall()
        return [r["user_id"] for r in rows]


def mark_summary_sent(user_id: int, today: str):
    with get_conn() as conn:
        conn.execute("UPDATE users SET last_summary_date = ? WHERE user_id = ?", (today, user_id))
...
# ─── Платежи ───
def create_payment_record(payment_id: str, user_id: int, amount_usd: float, plan_days: int = 30):
    with get_conn() as conn:
        conn.execute(
            "INSERT OR REPLACE INTO payments (payment_id, user_id, amount_usd, status, created_at, plan_days) "
            "VALUES (?, ?, ?, 'waiting', ?, ?)",
            (payment_id, user_id, amount_usd, int(time.time()), plan_days),
        )
