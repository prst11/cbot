...
import db
import payments
from shared import COINS, COIN_DISPLAY, display_coin, get_prices, get_price_history, DISPLAY_CURRENCIES, get_news, get_suggested_alert_percent
...
@app.post("/api/theme")
async def api_set_theme(body: ThemeIn, x_telegram_init_data: str = Header(default="")):
    user = get_current_user(x_telegram_init_data)
    if body.theme not in ("dark", "light"):
        raise HTTPException(400, "Некорректная тема")
    db.set_theme(user["id"], body.theme)
    return {"ok": True}
...
async def api_get_portfolio(bucket: str = "holding", x_telegram_init_data: str = Header(default="")):
    user = get_current_user(x_telegram_init_data)
    if bucket not in ("holding", "trading"):
        raise HTTPException(400, "Некорректная корзина")
    currency = db.get_currency(user["id"])
    items = db.get_portfolio(user["id"], bucket=bucket)
    ...
    return {
        "items": result, "total_value": total, "currency": currency, "bucket": bucket,
        "limit": FREE_PORTFOLIO_LIMIT, "unlimited": is_unlimited(user),
        "total_count": db.count_portfolio(user["id"]),
    }
...
async def api_add_portfolio(body: PortfolioIn, x_telegram_init_data: str = Header(default="")):
    ...
    if body.bucket not in ("holding", "trading"):
        raise HTTPException(400, "Некорректная корзина")
    ...
    db.add_portfolio_item(user["id"], body.coin, body.amount, bucket=body.bucket)
    return {"ok": True}
...
@app.get("/api/alerts/suggest")
async def api_suggest_alert_percent(coin: str, x_telegram_init_data: str = Header(default="")):
    get_current_user(x_telegram_init_data)
    coin_id = COINS.get(coin.upper())
    if not coin_id:
        raise HTTPException(404, "Неизвестная монета")
    percent = await get_suggested_alert_percent(coin_id)
    return {"coin": coin.upper(), "suggested_percent": percent}
...
async def api_premium_plans(x_telegram_init_data: str = Header(default="")):
    get_current_user(x_telegram_init_data)
    monthly_equiv = PREMIUM_PRICE_USD * 12
    savings = round(100 - (PREMIUM_YEARLY_PRICE_USD / monthly_equiv * 100)) if monthly_equiv else 0
    return {
        "month": {"price": PREMIUM_PRICE_USD, "days": PREMIUM_DAYS},
        "year": {"price": PREMIUM_YEARLY_PRICE_USD, "days": PREMIUM_YEARLY_DAYS, "savings_percent": savings},
    }
...
async def api_create_invoice(body: InvoiceIn, x_telegram_init_data: str = Header(default="")):
    user = get_current_user(x_telegram_init_data)
    is_yearly = body.plan == "year"
    ...
    try:
        invoice = payments.create_invoice(
            amount_usd=amount,
            order_id=order_id,
            description=f"Premium {days}d for user {user['id']}",
        )
    except Exception:
        raise HTTPException(502, "Не получилось создать платёж")
    payment_id = str(invoice.get("id") or invoice.get("payment_id") or order_id)
    db.create_payment_record(payment_id, user["id"], amount, plan_days=days)
    return {"payment_id": payment_id, "invoice_url": invoice.get("invoice_url"), "amount": amount, "days": days}
...
async def api_check_payment(payment_id: str, x_telegram_init_data: str = Header(default="")):
    ...
    if payments.is_payment_successful(status_data):
        ...
        new_until = db.activate_premium(user["id"], days)
        return {"confirmed": True, "premium_until": new_until, "days": days}
