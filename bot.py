...
import db
import payments
from shared import COINS, COIN_DISPLAY, display_coin, get_prices
...
async def admin_grant_start(call: CallbackQuery, state: FSMContext):
    if call.from_user.id not in admin_ids:
        ...
        return
    await call.message.edit_text(
        "🎟 Введи user_id и количество попыток через пробел, например:\n`123456789 10`\n\n"
...
async def admin_grant_process(message: Message, state: FSMContext):
    ...
    if len(parts) != 2 or not parts[0].isdigit() or not parts[1].lstrip("-").isdigit():
        await message.answer("❌ Формат: `user_id количество`, например: `123456789 10`", parse_mode="Markdown")
        return
...
async def add_alert_step_kind(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "akind_price":
        ...
        await call.message.edit_text(t(uid, "enter_percent").format(coin=display_coin(coin)), parse_mode="Markdown")
...
async def show_portfolio(call: CallbackQuery):
    ...
    for item in items:
        ...
        total += value
        arrow = "🟢" if change >= 0 else "🔴"
        text += t(uid, "portfolio_item").format(arrow=arrow, coin=display_coin(item["coin"]), amount=item["amount"], value=value, change=change)
