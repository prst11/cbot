...
async def get_suggested_alert_percent(coin_id: str) -> float:
    """
    Оценивает типичную дневную волатильность монеты за последние 7 дней
    (средний модуль дневного изменения цены) и предлагает разумный порог для алерта в %.
    Если история недоступна — отдаёт консервативное значение по умолчанию.
    """
    try:
    ...
    if len(history) < 2:
        return 5.0
    prices = [p[1] for p in history if p and p[1]]
    changes = []
    for i in range(1, len(prices)):
        prev = prices[i - 1]
        if prev:
            changes.append(abs((prices[i] - prev) / prev * 100))
    if not changes:
        return 5.0
    avg = sum(changes) / len(changes)
    return max(1.0, min(20.0, round(avg, 1)))
